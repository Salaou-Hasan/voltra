//! Procedural macros for NeonDB.
//!
//! These are re-exported through the `neondb` crate — users should import via
//! `neondb::reducer` / `neondb::table` rather than depending on this crate
//! directly.
...
/// Marks a function as a NeonDB native reducer.
///
...
/// `&mut ::neondb::reducer::context::ReducerContext`.  Name it `ctx` and use
...
/// Marks a struct as a NeonDB table row type.
///
