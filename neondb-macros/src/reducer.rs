...
fn expand_inner(input: ItemFn) -> Result<TokenStream, syn::Error> {
    ...
    let inner_fn = Ident::new(
        &format!("__neondb_reducer_{}", fn_name_s),
        Span::call_site(),
    );
    ...
    let expanded = quote! {
        ...
        fn #inner_fn(
            #ctx_name: &mut ::neondb::reducer::context::ReducerContext,
            args: &[u8],
        ) -> ::neondb::error::Result<::std::vec::Vec<u8>> {
        ...
        impl ::neondb::reducer::backend::ReducerBackend for #struct_name {
            fn execute(
                &self,
                ctx: &mut ::neondb::reducer::context::ReducerContext,
                args: &[u8],
            ) -> ::neondb::error::Result<::std::vec::Vec<u8>> {
                #inner_fn(ctx, args)
            }
        ...
        ::neondb::inventory::submit! {
            ::neondb::reducer::registry::NativeReducerItem {
                name: #fn_name_lit,
