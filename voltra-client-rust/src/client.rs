// ============================================================================
// Voltra Rust Client SDK — VoltraClient
...
/// Async Rust client for Voltra.
///
/// # Example
/// ```no_run
/// use voltra_client::{VoltraClient, ClientOptions, ReconnectConfig};
...
impl VoltraClient {
    /// Connect to Voltra and return a client handle.
...
async fn run_connection_with_reconnect(
    ...
    loop {
        ...
        if let Some(max) = reconnect.max_attempts {
            if attempt >= max {
                log::warn!(
                    "[voltra-client] Reconnect exhausted after {} attempts",
                    attempt
                );
        ...
        log::debug!(
            "[voltra-client] Reconnecting in {}ms (attempt {})",
            delay.as_millis(),
            attempt + 1
        );
        ...
        match make_ws_connection(&url, api_key.as_deref()).await {
            Err(e) => {
                log::warn!(
                    "[voltra-client] Reconnect attempt {} failed: {}",
                    attempt,
                    e
                );
                // Loop and try again.
            ...
            Ok(ws) => {
                ...
                log::info!("[voltra-client] Reconnected (attempt {})", attempt);
        ...
        tokio::select! {
            ...
            frame = stream.next() => {
                match frame {
                    None => break,
                    Some(Err(e)) => {
                        log::warn!("[voltra-client] WebSocket error: {}", e);
                        break;
                    }
...
fn dispatch_message(
    ...
    match msg {
        ...
        ServerMessage::SubscriptionAck(ack) => {
            if !ack.success {
                log::warn!(
                    "[voltra-client] Subscription '{}' failed: {:?}",
                    ack.subscription_id,
                    ack.message
                );
        ...
        ServerMessage::BatchUpdate { compressed, payload } => {
            let raw = if compressed {
                match zstd::decode_all(payload.as_slice()) {
                    Ok(out) => out,
                    Err(e) => {
                        log::warn!("[voltra-client] BatchUpdate zstd decompress failed: {}", e);
                        return;
                    }
                }
            } else {
