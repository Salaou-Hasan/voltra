// ============================================================================
// .vol DSL — Rust code generator
// ============================================================================
//
// Emits a `reducers.rs` file using `#[voltra::reducer]` macros.
//
// Symbol table tracks:
//   Param(ty)          — reducer parameter with known Voltra type
...
pub fn generate(program: &Program) -> Result<String, VoltraError> {
    let mut out = String::new();

    out.push_str("// Auto-generated by the Voltra .vol compiler. Do not edit.\n\n");
    out.push_str("use voltra::reducer::context::ReducerContext;\n\n");
...
fn gen_reducer(r: &ReducerDecl, scope: &mut Scope) -> Result<String, VoltraError> {
    let mut out = String::new();

    out.push_str("#[allow(unused_variables, dead_code, unused_mut)]\n");
    out.push_str("#[voltra::reducer]\n");
    ...
    match stmt {
        ...
        Stmt::LetRow { name, table, key, else_body, line: _ } => {
            ...
            if let Some(body) = else_body {
                ...
                out.push_str(&format!(
                    "{pad}        return Err(voltra::error::VoltraError::reducer_error(\
...
fn gen_builtin_call(name: &str, args: &[String]) -> String {
    ...
    match name {
        ...
        // ── Random (timestamp + atomic counter seed — unique across same-nanosecond calls) ──
        "rand_int"    => format!(
            "{{ \
             static __RSEQ: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0); \
             let __c = __RSEQ.fetch_add(1, std::sync::atomic::Ordering::Relaxed); \
             let __ts = ctx.timestamp() ^ __c.wrapping_mul(0x9e3779b97f4a7c15u64); \
             let __lo = {} as i64; let __hi = {} as i64; \
        ...
        "rand_float"  => "{ \
             static __RSEQF: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(1); \
             let __c = __RSEQF.fetch_add(1, std::sync::atomic::Ordering::Relaxed); \
             let __ts = ctx.timestamp() ^ __c.wrapping_mul(0x9e3779b97f4a7c15u64); \
        ...
        "slice"         => format!(
            "serde_json::Value::Array(({}).as_array().map(|__v| {{\
             let __s = (({}) as i64).max(0) as usize;\
             let __e = (({}) as i64).max(0) as usize;\
             let __e = __e.min(__v.len()).max(__s);\
             __v[__s..__e].to_vec()\
             }}).unwrap_or_default())",
            a(0), a(1), a(2)
        ),
        ...
        "cluster_route" => format!(
            "(voltra::cluster::cluster_shard_for_key(&({}).to_string()) as i64)",
            a(0)
        ),
        ...
        "cross_cluster_call" => format!(
            "{{ \
             let __cc_bytes = voltra::cluster::proxy_call_global(\
               ({}) as u32, \
               &({}).to_string(), \
               &({}).to_string(), \
               &ctx.caller_id, \
        ...
        "region_count_rows" => format!(
            "{{ \
             ...
             voltra::cluster::region_count_rows_global(&({}).to_string(), __local_n) \
             }}",
            a(0), a(0)
        ),
        ...
        "migrate_to_cluster" => format!(
            "{{ \
             ...
             let __mt_ok = voltra::cluster::migrate_row_global(\
               &({}).to_string(), \
               &({}).to_string(), \
               ({}) as u32, \
               __mt_data\
...
mod tests {
    ...
    #[test]
    fn codegen_letrow_no_unreachable_after_error() {
    ...
    fn codegen_empty_reducer() {
        let out = compile("reducer reset() {}");
        assert!(out.contains("#[voltra::reducer]"));
    ...
    fn codegen_return_row_literal() {
        ...
        assert!(out.contains("voltra::ret!"));
    ...
    #[test]
    fn codegen_smoke_voltra_file() {
        let src = include_str!("tests/smoke_test.vol");
    ...
    fn codegen_error_stmt() {
        ...
        assert!(out.contains("VoltraError::reducer_error"));
        assert!(out.contains("boom"));
    }

    #[test]
    fn codegen_mmo_full_test_voltra() {
        let src = include_str!("tests/mmo_full_test.vol");
