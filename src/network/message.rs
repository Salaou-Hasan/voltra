...
pub struct ReducerCall {
    ...
    /// Client assigns monotonically increasing numbers; server echoes it back
    /// in the ReducerResponse so the client can reconcile predicted state.
...
pub struct ReducerResponse {
    ...
    /// Echoed sequence number from the client's ReducerCall.
    /// `None` for server-initiated responses (scheduled reducers, admin calls).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub sequence: Option<u64>,
    /// Server tick number when this response was generated.
    /// Clients use this for interpolation and prediction reconciliation.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl ReducerResponse {
    ...
    pub fn with_sequence(mut self, seq: Option<u64>) -> Self {
        self.sequence = seq;
        self
    }

    pub fn with_server_tick(mut self, tick: u64) -> Self {
