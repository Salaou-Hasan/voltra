            ...
            Ok(Message::Binary(data)) => {
                match protocol::decode_client_message(&data) {
                    ...
                    Ok(ClientMessage::Subscribe { subscription_id, query }) => {
                        // Check for AOI subscription: "__aoi WHERE lobby = 'l0'"
                        let trimmed = query.trim();
                        if trimmed.starts_with("__aoi") {
                            // Extract lobby ID from WHERE clause
                            let lobby_id = trimmed
                                .split_whitespace()
                                .skip_while(|s| *s != "WHERE")
                                .skip(1)
                                .next()
                                .and_then(|col| {
                                    // expect: lobby = 'l0' or lobby='l0'
                                    let eq_pos = col.find('=')?;
                                    let val = col[eq_pos + 1..].trim().trim_matches('\'').trim_matches('"');
                                    Some(val.to_string())
                                });

                            if let (Some(lid), Some(lr)) = (&lobby_id, &lobby_router) {
                                if let Some(broadcaster) = lr.get_aoi_broadcaster(lid) {
                                    // Register client for AOI broadcast using the same sub_tx
                                    // The sub_tx was already moved into sub_task, so we create
                                    // a new channel and bridge it.
                                    let (aoi_tx, mut aoi_rx) = mpsc::channel::<OutboundFrames>(CLIENT_SEND_BUFFER_CAPACITY);
                                    let write_tx_bridge = write_tx.clone();
                                    let _bridge_task = tokio::spawn(async move {
                                        while let Some(frames) = aoi_rx.recv().await {
                                            match frames {
                                                OutboundFrames::One(bytes) => {
                                                    let _ = write_tx_bridge.try_send(Message::Binary(bytes.to_vec()));
                                                }
                                                OutboundFrames::Two { first, second } => {
                                                    let _ = write_tx_bridge.try_send(Message::Binary(first.to_vec()));
                                                    let _ = write_tx_bridge.try_send(Message::Binary(second.to_vec()));
                                                }
                                            }
                                        }
                                    });
                                    // Store the bridge handle for cleanup
                                    // For now, just register with a dummy player entity (0,0)
                                    // The real player entity will be set when they spawn
                                    let player_entity = crate::runtime::ecs::EntityId::new(0, 0);
                                    broadcaster.register_client(client_id, aoi_tx, player_entity);
                                    log::info!("Client {} subscribed to AOI lobby {}", client_id, lid);

                                    let ack = ServerMessage::SubscriptionAck {
                                        subscription_id,
                                        success: true,
                                        message: Some(format!("aoi:{}", lid)),
                                    };
                                    if let Ok(encoded) = protocol::encode_server_message(&ack) {
                                        let _ = write_tx.try_send(Message::Binary(encoded));
                                    }
                                    // Store bridge task for cleanup (best-effort, drops on disconnect)
                                    // TODO: track bridge_tasks per client for clean unsubscribe
                                    continue;
                                }
                            }
                            // Fall through to normal subscription if AOI setup fails
                        }
