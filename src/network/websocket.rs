...
pub struct PendingCall {
    ...
    /// Ignored for ordinary single-call `PendingCall`s. Unbounded is fine
    /// here — a transaction's outcome is a single message, not an unbounded
    /// stream, so this can't be a backpressure/memory-growth vector the way
    /// per-call `response_tx` could be.
