...
impl LobbyRuntime {
    ...
    pub fn register_aoi_client(
        ...
        self.broadcaster
            .register_client(client_id, tx, player_entity);
    }
    ...
    pub fn spawn_player(&self, session_id: u64, name: &str, x: f32, y: f32, hp: i32) -> EntityId {
...
mod tests {
    ...
    fn lobby_runtime_despawn_removes_from_world_and_aoi() {
        ...
        assert!(
            !rt.aoi().read().visible_to(e).is_empty() || rt.aoi().read().grid.position(e).is_none()
        );
    }
