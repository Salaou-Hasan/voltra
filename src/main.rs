...
enum Commands {
    ...
    Init {
        ...
        #[arg(long, help = "Genre recipe: fps | mmo | battle-royale | survival | racing | moba | social")]
        genre: Option<String>,
        #[arg(long, help = "Extra runtime modules to add (comma-separated)")]
        with_modules: Option<String>,
    },
...
async fn main() -> Result<()> {
    ...
    match command {
        Commands::Init { path, template, genre, with_modules } => { init_project(path, template, genre, with_modules)?; Ok(()) }
...
fn init_project(path: Option<PathBuf>, template: Option<String>, genre: Option<String>, with_modules: Option<String>) -> Result<()> {
    if template.is_some() && genre.is_some() {
        eprintln!("Error: --template and --genre are mutually exclusive. Use one or the other.");
        return Err(voltra::error::VoltraError::invalid_argument("cannot use both --template and --genre"));
    }
    ...
    if let Some(genre_id) = &genre {
        let extra: Vec<&str> = with_modules
            .as_deref()
            .map(|s| s.split(',').map(|s| s.trim()).filter(|s| !s.is_empty()).collect())
            .unwrap_or_default();
        return scaffold_genre(&project_path, &project_name, genre_id, &extra);
    }

    // ── Template-based init (existing) ──────────────────────────────────────────
...
/// Scaffold a genre-based project from the runtime composition model.
fn scaffold_genre(project_path: &Path, project_name: &str, genre_id: &str, extra_modules: &[&str]) -> Result<()> {
    let composition = runtime::compose_runtime(Some(genre_id), extra_modules)
        .map_err(|e| voltra::error::VoltraError::invalid_argument(e))?;

    fs::create_dir_all(project_path)
        .map_err(|e| voltra::error::VoltraError::internal(format!("Cannot create directory: {}", e)))?;

    // Write shared scaffold files (voltra.toml, .gitignore, migrations/, modules/)
    // Use a dummy template name — genre projects don't match any TEMPLATES entry.
    write_shared_files(project_path, project_name, "_genre")?;

    // Cargo.toml (Voltra library crate)
    wf(project_path, "Cargo.toml", &game_cargo_toml(project_name))?;
    copy_lockfile_if_available(project_path)?;
    wf(project_path, "rust-toolchain.toml", RUST_TOOLCHAIN)?;

    // List of resolved module IDs
    let module_ids: Vec<&str> = composition.modules.iter().map(|m| m.id).collect();

    // Build the mod.rs that declares all reducer submodules
    let mut mod_rs = String::from("// Auto-generated by `voltra init --genre` — add your reducers here\n\n");
    for id in &module_ids {
        mod_rs.push_str(&format!("pub mod {};\n", id.replace('-', "_")));
    }

    // Build the main.rs that boots the embedded Voltra server
    let main_rs = format!(
        "use voltra::server::run_server;\nuse voltra::config::Config;\n\n\
         #[tokio::main]\nasync fn main() -> voltra::error::Result<()> {{\n\
         \x20\x20println!(\"Starting {{}}...\", env!(\"CARGO_PKG_NAME\"));\n\
         \x20\x20let config = Config::from_env();\n\
         \x20\x20run_server(config).await\n\
         }}\n"
    );
    wf(project_path, "src/main.rs", &main_rs)?;
    wf(project_path, "src/reducers/mod.rs", &mod_rs)?;

    // Generate a placeholder reducer for each module
    for id in &module_ids {
        let reducer_code = genre_reducer_for_module(id);
        let path = format!("src/reducers/{}.rs", id.replace('-', "_"));
        wf(project_path, &path, &reducer_code)?;
    }

    // schema.toml with a basic players table
    let mut schema = String::from("[players]\n\n[players.columns]\n");
    schema.push_str("hp = \"i32\"\nalive = \"bool\"\nzone = \"string\"\n\n");
    for id in &module_ids {
        schema.push_str(&genre_schema_for_module(id));
    }
    wf(project_path, "schema.toml", &schema)?;
    ...
    let readme = format!(
        ...
        genre_id,
        module_ids.join(", "),
        module_ids.iter().map(|id| format!("- `{}`", id)).collect::<Vec<_>>().join("\n"),
    );
    wf(project_path, "README.md", &readme)?;

    // Client SDKs
    scaffold_all_clients(project_path, project_name)?;

    // Print success
    println!();
    println!("  Genre '{}' project ready!\n", genre_id);
    println!("  Files written:");
    println!("    {:<40} {}", "Cargo.toml", "Voltra embedded game server");
    println!("    {:<40} {}", "src/main.rs", "Server bootstrap");
    println!("    {:<40} {}", "schema.toml", "Table definitions");
    for id in &module_ids {
        let path = format!("src/reducers/{}.rs", id.replace('-', "_"));
        let desc = composition.modules.iter()
            .find(|m| m.id == *id)
            .map(|m| m.description)
            .unwrap_or("module");
        println!("    {:<40} {}", path, desc);
    }
    println!();
    println!("  Next steps:");
...
/// Generate a placeholder reducer file for a given runtime module ID.
fn genre_reducer_for_module(id: &str) -> String {
    match id {
        "sessions" => "// Session validation is handled by the server.\n// This module is a control-plane marker.\n".to_string(),
        "lobby" => "// Lobby ownership and routing — extend with your lobby logic.\n".to_string(),
        "tick" => "// Per-lobby tick scheduler.\n// Hook into the scheduled reducer system in voltra.toml.\n".to_string(),
        "ecs" => "// Dense ECS component storage lives in-memory.\n// Reducers here project durable state into the hot ECS.\n".to_string(),
        "aoi" => "// Area-of-interest filtering — spatial subscriptions.\n".to_string(),
        "delta" => "// Delta encoding and fanout — managed by the server runtime.\n".to_string(),
        "runtime-persistence" => "// Runtime log and lobby snapshots — append-only persistence.\n".to_string(),
        "movement" => "use voltra::{reducer, ret};\nuse voltra::reducer::ReducerContext;\n\n#[reducer]\npub fn move_player(ctx: &mut ReducerContext, player_id: String, x: f64, y: f64) {\n    let row = ctx.get_row(\"players\", &player_id)?.unwrap_or_else(|| serde_json::json!({}));\n    let mut obj = row.as_object().cloned().unwrap_or_default();\n    obj.insert(\"x\".into(), serde_json::json!(x));\n    obj.insert(\"y\".into(), serde_json::json!(y));\n    ctx.set_row(\"players\", &player_id, serde_json::Value::Object(obj))?;\n    ret!({ \"ok\": true, \"x\": x, \"y\": y })\n}\n".to_string(),
        "weapons" => "// Weapon state, fire commands, cooldowns, and ammo.\n// Extend with your weapon system logic.\n".to_string(),
        "combat" => "use voltra::{reducer, ret};\nuse voltra::reducer::ReducerContext;\n\n#[reducer]\npub fn attack(ctx: &mut ReducerContext, attacker_id: String, target_id: String, damage: i32) {\n    let target = ctx.get_row(\"players\", &target_id)?.unwrap_or_else(|| serde_json::json!({ \"hp\": 100 }));\n    let hp = target[\"hp\"].as_i64().unwrap_or(100) - damage as i64;\n    let mut obj = target.as_object().cloned().unwrap_or_default();\n    obj.insert(\"hp\".into(), serde_json::json!(hp.max(0)));\n    ctx.set_row(\"players\", &target_id, serde_json::Value::Object(obj))?;\n    ret!({ \"ok\": true, \"damage\": damage, \"target_hp\": hp.max(0) })\n}\n".to_string(),
        "hit-detection" => "// Latency-aware hit validation and collision queries.\n// Server-side: extend with your hit validation logic.\n".to_string(),
        "inventory" => "use voltra::{reducer, ret};\nuse voltra::reducer::ReducerContext;\n\n#[reducer]\npub fn add_item(ctx: &mut ReducerContext, player_id: String, item_id: String, quantity: i32) {\n    let key = format!(\"{}_{}\", player_id, item_id);\n    ctx.set_row(\"inventory\", &key, serde_json::json!({\n        \"player_id\": player_id, \"item_id\": item_id, \"quantity\": quantity\n    }))?;\n    ret!({ \"ok\": true })\n}\n\n#[reducer]\npub fn remove_item(ctx: &mut ReducerContext, player_id: String, item_id: String, quantity: i32) {\n    let key = format!(\"{}_{}\", player_id, item_id);\n    let row = ctx.get_row(\"inventory\", &key)?.unwrap_or_else(|| serde_json::json!({ \"quantity\": 0 }));\n    let current = row[\"quantity\"].as_i64().unwrap_or(0) - quantity as i64;\n    if current <= 0 {\n        ctx.delete_row(\"inventory\", &key)?;\n    } else {\n        ctx.set_row(\"inventory\", &key, serde_json::json!({\n            \"player_id\": player_id, \"item_id\": item_id, \"quantity\": current\n        }))?;\n    }\n    ret!({ \"ok\": true })\n}\n".to_string(),
        "equipment" => "// Equipped items with hot stat projection into ECS.\n// Extend with your equipment slot logic.\n".to_string(),
        "economy" => "use voltra::{reducer, ret};\nuse voltra::reducer::ReducerContext;\n\n#[reducer]\npub fn transfer_gold(ctx: &mut ReducerContext, from_id: String, to_id: String, amount: i64) {\n    let from = ctx.get_row(\"players\", &from_id)?.unwrap_or_else(|| serde_json::json!({ \"gold\": 0 }));\n    let to = ctx.get_row(\"players\", &to_id)?.unwrap_or_else(|| serde_json::json!({ \"gold\": 0 }));\n    let from_gold = from[\"gold\"].as_i64().unwrap_or(0);\n    let to_gold = to[\"gold\"].as_i64().unwrap_or(0);\n    if from_gold < amount { ret!({ \"ok\": false, \"error\": \"insufficient gold\" }) }\n    let mut from_obj = from.as_object().cloned().unwrap_or_default();\n    let mut to_obj = to.as_object().cloned().unwrap_or_default();\n    from_obj.insert(\"gold\".into(), serde_json::json!(from_gold - amount));\n    to_obj.insert(\"gold\".into(), serde_json::json!(to_gold + amount));\n    ctx.set_row(\"players\", &from_id, serde_json::Value::Object(from_obj))?;\n    ctx.set_row(\"players\", &to_id, serde_json::Value::Object(to_obj))?;\n    ret!({ \"ok\": true })\n}\n".to_string(),
        "quests" => "// Quest state, progress tracking, and rewards.\n// Extend with your quest system logic.\n".to_string(),
        "guilds" => "use voltra::{reducer, ret};\nuse voltra::reducer::ReducerContext;\n\n#[reducer]\npub fn create_guild(ctx: &mut ReducerContext, guild_id: String, name: String, owner_id: String) {\n    ctx.set_row(\"guilds\", &guild_id, serde_json::json!({\n        \"name\": name, \"owner_id\": owner_id, \"members\": [owner_id]\n    }))?;\n    ret!({ \"ok\": true })\n}\n".to_string(),
        "parties" => "// Party queueing, invites, and shared matchmaking entry.\n// Extend with your party system logic.\n".to_string(),
        "matchmaking" => "// Queues, reservations, backfill, and lobby allocation.\n// Extend with your matchmaking logic.\n".to_string(),
        "chat" => "// Chat rooms, messages, reactions, moderation, and presence.\n// Use the built-in chat reducers from `voltra add chat`.\n".to_string(),
        "leaderboard" => "// Score submission, ranking, seasons, and resets.\n// Extend with your leaderboard logic.\n".to_string(),
        "replay" => "// Tick input/delta capture for debugging and spectating.\n// Extend with your replay system logic.\n".to_string(),
        _ => format!("// {} — placeholder reducer module.\n", id),
    }
}

/// Generate schema.toml additions for a given module.
fn genre_schema_for_module(id: &str) -> String {
    match id {
        "inventory" => "\n[inventory]\n\n[inventory.columns]\nplayer_id = \"string\"\nitem_id = \"string\"\nquantity = \"i32\"\n\n",
        "guilds" => "\n[guilds]\n\n[guilds.columns]\nname = \"string\"\nowner_id = \"string\"\nmembers = \"string\"\n\n",
        "quests" => "\n[quests]\n\n[quests.columns]\nplayer_id = \"string\"\nquest_id = \"string\"\nprogress = \"i32\"\ncompleted = \"bool\"\n\n",
...
async fn run_server(config: Config) -> Result<()> {
    ...
    let lobby_router = {
        let lobby_tick_config = if config.lobby_tick_hz > 0 {
            Some(voltra::runtime::tick::TickConfig {
                tick_rate_hz: config.lobby_tick_hz,
                max_tick_time_ms: 10,
            })
        } else {
            None
        ...
        let worker_deps = std::sync::Arc::new(voltra::worker_pool::WorkerDeps {
            ...
            lobby_tick_config,
            lobby_view_distance: config.lobby_view_distance,
        });
...
struct AdminState {
    ...
    /// Snapshot directory — used by /stats to report snapshot_last_seq.
    snapshot_dir: PathBuf,
}
    ...
    match (req.method(), path.as_str()) {
        ...
        (&Method::GET, "/stats") => {
            ...
            Ok(json_response(serde_json::json!({
                ...
                "wal_sequence": global_seq.load(std::sync::atomic::Ordering::Relaxed),
                "wal_file_size_bytes": wal_writer.wal_file_size_bytes(),
                "snapshot_last_seq": find_latest_snapshot(&admin.snapshot_dir).map(|(_, s)| s).unwrap_or(0),
            })))
        },
