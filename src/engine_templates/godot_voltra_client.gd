# ==============================================================================
# Voltra Godot 4 Client — single-file, zero dependencies.
#
# Add as an autoload (Project Settings → Autoload → this script as "Voltra"),
# or attach to a Node. Then:
#
#   Voltra.connect_to("ws://127.0.0.1:3000")
#   await Voltra.connected
#
#   var result = await Voltra.call_reducer("spawn", ["player1", 0, 0, "warrior"])
#   print(result.success, " ", result.data)
#
#   Voltra.subscribe("players WHERE zone = 'z_0_0'")
#   # then connect the row_update signal:
#   Voltra.row_update.connect(func(table, key, op, data):
#       print(op, " ", key, ": ", data))
#
...
func connect_to(url: String, api_key: String = "") -> void:
	...
	if err != OK:
		push_error("[Voltra] connect failed: %s" % err)
	set_process(true)
...
##   var r = await Voltra.call_reducer("spawn", ["p1", 0, 0, "warrior"])
