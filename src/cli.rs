...
///
/// Accepts either a VQL string or a path to a `.vql` file.
/// Sends the query to `POST /admin/api/vql` on the metrics port.
pub async fn cmd_query(metrics_url: &str, query: &str, api_key: Option<&str>) -> Result<()> {
    // If the query looks like a file path ending in .vql, read it
    let vql_source = if query.ends_with(".vql") || query.ends_with(".VQL") {
        std::fs::read_to_string(query)
            .map_err(|e| VoltraError::invalid_argument(format!("Failed to read VQL file '{}': {}", query, e)))?
    } else {
        query.to_string()
    };

    let url = format!("{}/admin/api/vql", metrics_url.trim_end_matches('/'));
    let client = reqwest::Client::new();
    let mut req = client.post(&url)
        .json(&serde_json::json!({ "query": &vql_source }));
    ...
    match req.send().await {
            ...
            match resp.text().await {
                Ok(body) => {
                    ...
                    if let Ok(val) = serde_json::from_str::<serde_json::Value>(&body) {
                        // Print columns header if present
                        if let Some(cols) = val.get("columns").and_then(|c| c.as_array()) {
                            if !cols.is_empty() {
                                let header: Vec<String> = cols.iter()
                                    .filter_map(|c| c.as_str().map(|s| s.to_string()))
                                    .collect();
                                println!("{}", header.join(" | "));
                                println!("{}", header.iter().map(|h| "-".repeat(h.len().max(3))).collect::<Vec<_>>().join(" | "));
                            }
                        }
                        // Print rows
                            ...
                            if rows.is_empty() {
                                ...
                                for row in rows {
                                    if let Some(obj) = row.as_object() {
                                        let vals: Vec<String> = obj.values()
                                            .map(|v| match v {
                        ...
                        // Print rows_affected for DML
                        if let Some(affected) = val.get("rows_affected").and_then(|v| v.as_u64()) {
                            if affected > 0 {
                                println!("({} row{} affected)", affected, if affected == 1 { "" } else { "s" });
                            }
                        }
                    } else {
                        print_json_pretty(&body);
        ...
        Err(e) => {
            eprintln!("Failed to connect to server at {}: {}", metrics_url, e);
            eprintln!("Is the server running? Start it with: voltra start");
        }
    }
    Ok(())
}
