        ...
        tokio::task::spawn_blocking(move || {
            ...
            loop {
                ...
                for call in batch {
                    ...
                    if let Some(batch_calls) = call.tx_batch {
                                ...
                                match commit_result {
                                    Ok(deltas) => {
                                        ...
                                        if let Some(ref pe) = persist_w {
                                            if !deltas.is_empty() {
                                                if let Err(e) =
                                                    pe.persist_deltas(&deltas, seq_num)
                                                {
                                                    log::warn!(
                        ...
                        if let Some(tx_tx) = &call.tx_response_tx {
                            if let Err(e) = tx_tx.send(outcome) {
                                log::warn!(
                                    "[voltra] Transaction result delivery failed: {}",
                                    e
                                );
                            }
                        }
                    ...
                    let response = loop {
                        attempt += 1;
                        let exec = tracing::info_span!("dispatch", attempt)
                            .in_scope(|| registry_w.execute(&call.reducer_name, &mut ctx, &call.args));
