...
pub fn decode_field_from_bytes(data: &[u8], field: &str) -> Result<Option<Value>> {
    ...
    let raw = match tag {
        ...
        _ => return Err(VoltraError::SerializationError(format!(
            "Row decode: unknown tag 0x{:02x}", tag
        ))),
    };
...
pub fn decode_fields_from_bytes(data: &[u8], fields: &[&str]) -> Result<Option<serde_json::Map<String, Value>>> {
    let (tag, payload) = data.split_first().ok_or_else(|| {
        VoltraError::SerializationError("Row decode: empty data".to_string())
    })?;
    match tag {
...
fn decode_fields_from_decompressed(data: &[u8], fields: &[&str]) -> Result<Option<serde_json::Map<String, Value>>> {
    if fields.is_empty() { return Ok(Some(serde_json::Map::new())); }
    ...
    if found > 0 { Ok(Some(result)) } else { Ok(None) }
}
...
fn parse_map_header(data: &[u8], pos: &mut usize) -> Option<usize> {
    ...
    match marker {
        // fixmap: 0x80..=0x8f
        0x80..=0x8f => { *pos += 1; Some((marker & 0x0f) as usize) }
        // map16: 0xde
        0xde => {
...
fn skip_msgpack_value(data: &[u8], pos: &mut usize) {
    ...
    match marker {
        ...
        0xc3 => { *pos += 1; }
        // bin 8/16/32
        0xc4 => { *pos += 2; if *pos <= data.len() { let l = data[*pos - 1] as usize; *pos += l.min(data.len() - *pos); } }
        0xc5 => { if *pos + 3 <= data.len() { let l = u16::from_be_bytes([data[*pos+1], data[*pos+2]]) as usize; *pos += 3 + l.min(data.len() - *pos - 3); } }
        0xc6 => { if *pos + 5 <= data.len() { let l = u32::from_be_bytes([data[*pos+1], data[*pos+2], data[*pos+3], data[*pos+4]]) as usize; *pos += 5 + l.min(data.len() - *pos - 5); } }
        // float32, float64
        0xca => { *pos += 5; }
        0xcb => { *pos += 9; }
        ...
        // str8/16/32
        0xd9 => { if *pos + 2 <= data.len() { let l = data[*pos+1] as usize; *pos += 2 + l.min(data.len() - *pos - 2); } }
        0xda => { if *pos + 3 <= data.len() { let l = u16::from_be_bytes([data[*pos+1], data[*pos+2]]) as usize; *pos += 3 + l.min(data.len() - *pos - 3); } }
        0xdb => { if *pos + 5 <= data.len() { let l = u32::from_be_bytes([data[*pos+1], data[*pos+2], data[*pos+3], data[*pos+4]]) as usize; *pos += 5 + l.min(data.len() - *pos - 5); } }
        // fixarray: 0x90..=0x9f
        0x90..=0x9f => {
            let len = (marker & 0x0f) as usize;
            *pos += 1;
            for _ in 0..len { skip_msgpack_value(data, pos); }
        }
        // array16/32
        0xdc => { if *pos+3 <= data.len() { let l = u16::from_be_bytes([data[*pos+1],data[*pos+2]]) as usize; *pos+=3; for _ in 0..l { skip_msgpack_value(data,pos); } } }
        0xdd => { if *pos+5 <= data.len() { let l = u32::from_be_bytes([data[*pos+1],data[*pos+2],data[*pos+3],data[*pos+4]]) as usize; *pos+=5; for _ in 0..l { skip_msgpack_value(data,pos); } } }
        // fixmap: 0x80..=0x8f
        0x80..=0x8f => {
            let len = (marker & 0x0f) as usize;
            *pos += 1;
            for _ in 0..len { skip_msgpack_value(data, pos); skip_msgpack_value(data, pos); }
        }
        // map16/32
        0xde => { if *pos+3 <= data.len() { let l = u16::from_be_bytes([data[*pos+1],data[*pos+2]]) as usize; *pos+=3; for _ in 0..l { skip_msgpack_value(data,pos); skip_msgpack_value(data,pos); } } }
        0xdf => { if *pos+5 <= data.len() { let l = u32::from_be_bytes([data[*pos+1],data[*pos+2],data[*pos+3],data[*pos+4]]) as usize; *pos+=5; for _ in 0..l { skip_msgpack_value(data,pos); skip_msgpack_value(data,pos); } } }
        // ext 8/16/32
        0xc7 => { if *pos+2<=data.len() { let l=data[*pos+1] as usize; *pos+=2+1+l.min(data.len()-*pos-2-1); } }
        0xc8 => { if *pos+3<=data.len() { let l=u16::from_be_bytes([data[*pos+1],data[*pos+2]]) as usize; *pos+=3+1+l.min(data.len()-*pos-3-1); } }
        0xc9 => { if *pos+5<=data.len() { let l=u32::from_be_bytes([data[*pos+1],data[*pos+2],data[*pos+3],data[*pos+4]]) as usize; *pos+=5+1+l.min(data.len()-*pos-5-1); } }
        _ => { *pos += 1; }
    }
}
