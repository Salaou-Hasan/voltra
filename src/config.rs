...
/// Server configuration loaded from `voltra.toml`, environment variables, or defaults.
#[derive(Clone, Debug)]
pub struct Config {
    pub host: String,
    pub port: u16,
    ...
    /// Env: VOLTRA_TWO_FRAME_PROTOCOL=1. Default: false (legacy one-frame mode).
    ...
    /// memory.  Default 16 MiB.  Env: `VOLTRA_MAX_BLOB_SIZE`.
    ...
    /// Default 64 MiB.  Env: `VOLTRA_REDUCER_MAX_MEMORY_BYTES`.
    ...
    /// Env: `VOLTRA_REDUCER_QUEUE_CAP`.  Default 16 384.
    ...
    /// scratch.  Env: `VOLTRA_PERSISTENCE_PATH`.  Default `None` (WAL-only mode).
    ...
    /// Required when role == "replica".  Env: `VOLTRA_PRIMARY_URL`.
    pub primary_url: Option<String>,
    /// How often (ms) a replica polls the primary for new WAL entries.
    /// Env: `VOLTRA_REPLICA_POLL_MS`.  Default 500.
    ...
    /// to primary and starts accepting writes.  Env: `VOLTRA_AUTO_FAILOVER`.
    /// Default false (manual `voltra promote` / POST /replication/promote).
    pub auto_failover: bool,
    /// Consecutive failed polls before an auto-failover promotion fires.
    /// Env: `VOLTRA_FAILOVER_MISS_COUNT`.  Default 3.
    ...
    /// copy) on that interval and rotates old ones.  Env: `VOLTRA_BACKUP_DIR`.
    pub backup_dir: Option<PathBuf>,
    /// Seconds between automated backups.  0 disables the background task
    /// (manual `POST /backup` still works).  Env: `VOLTRA_BACKUP_INTERVAL_SECS`.
    pub backup_interval_secs: u64,
    /// How many rotated backups to keep.  Env: `VOLTRA_BACKUP_KEEP`.  Default 5.
    pub backup_keep: usize,
    /// Worker thread count.  0 = auto (num_cpus, min 2).
    /// Env: `VOLTRA_WORKERS`.  Only increase beyond num_cpus for benchmarking.
    pub workers: usize,
    /// Redis protocol (RESP) listener port.  0 = disabled.
    /// Env: `VOLTRA_REDIS_PORT`.  Default 6379.
    pub redis_port: u16,
    /// Redis AUTH password.  None = no auth required.
    /// Env: `VOLTRA_REDIS_PASSWORD`.
    pub redis_password: Option<String>,
    /// PostgreSQL wire protocol listener port.  0 = disabled.
    /// Env: `VOLTRA_PG_PORT`.  Default 5432.
    pub pg_port: u16,
    /// PostgreSQL cleartext password.  None = trust auth.
    /// Env: `VOLTRA_PG_PASSWORD`.
    ...
    /// 0 = deliver every write immediately.  Env: `VOLTRA_SUB_TICK_MS`.
    pub sub_tick_ms: u64,
    /// Per-lobby game simulation tick rate (Hz). 0 = disabled (no tick loop).
    /// Env: `VOLTRA_LOBBY_TICK_HZ`.  Default 0 (off — enable for game lobbies).
    pub lobby_tick_hz: u32,
    /// AOI view distance for lobby runtimes.  Env: `VOLTRA_LOBBY_VIEW_DISTANCE`.  Default 200.0.
    pub lobby_view_distance: f32,
    ...
    /// Env: `VOLTRA_REGIONS`.  Default "" (single-region mode).
    pub regions: String,
    ...
    /// Env: `VOLTRA_LEADERBOARD_INTERVAL_SECS`.  Default 60.
    pub leaderboard_interval_secs: u64,
    /// How many entries to pull from each region.
    /// Env: `VOLTRA_LEADERBOARD_TOP_N`.  Default 1000.
...
struct ConfigServer {
    ...
    lobby_tick_hz: Option<u32>,
    lobby_view_distance: Option<f32>,
}
...
impl Config {
    /// Load configuration by searching for `voltra.toml` in current directory and parents,
    /// then merging environment variables and defaults.
    pub fn from_env() -> Self {
        let default_host = "127.0.0.1".to_string();
        let default_port = 3000u16;
        let default_wal = env::temp_dir().join("voltra.wal");
        ...
        let mut cfg = Config {
            ...
            snapshot_dir: env::temp_dir().join("voltra_snapshots"),
        ...
        if cfg.api_key.is_none() && cfg.host != "127.0.0.1" && cfg.host != "localhost" {
            log::warn!(
                "SECURITY WARNING: Voltra is binding to '{}' with NO api_key set. \
                 Any client on the network can call reducers. \
                 Set VOLTRA_API_KEY=<long-random-secret> or `[server] api_key = \"...\"` \
                 in voltra.toml before exposing this port. \
...
fn apply_server_section(cfg: &mut Config, server: Option<ConfigServer>) {
    ...
    if let Some(hz) = s.lobby_tick_hz {
        cfg.lobby_tick_hz = hz;
    }
    if let Some(d) = s.lobby_view_distance {
        cfg.lobby_view_distance = d.max(10.0);
    }
}
...
fn apply_env_overrides(cfg: &mut Config) {
    if let Ok(h) = env::var("VOLTRA_HOST") {
        cfg.host = h;
    }
    if let Ok(p) =
        env::var("VOLTRA_PORT").and_then(|v| v.parse().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.port = p;
    }
    if let Ok(w) = env::var("VOLTRA_WAL_PATH") {
        cfg.wal_path = PathBuf::from(w);
    }
    if let Ok(f) = env::var("VOLTRA_FSYNC_INTERVAL_MS")
        .and_then(|v| v.parse().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.fsync_interval_ms = f;
    }
    if let Ok(b) = env::var("VOLTRA_WAL_BATCH_SIZE")
        .and_then(|v| v.parse().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.wal_batch_size = b;
    }
    if let Ok(i) = env::var("VOLTRA_WAL_BATCH_INTERVAL_MS")
        .and_then(|v| v.parse().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.wal_batch_interval_ms = i;
    }
    if let Ok(v) = env::var("VOLTRA_UNSAFE_NO_FSYNC") {
        cfg.unsafe_no_fsync = v == "1" || v.eq_ignore_ascii_case("true");
    }
    if let Ok(s) = env::var("VOLTRA_SHARD_ID")
        .and_then(|v| v.parse().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.shard_id = s;
    }
    if let Ok(c) = env::var("VOLTRA_SHARD_COUNT")
        .and_then(|v| v.parse().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.shard_count = c;
    }
    if let Ok(p) = env::var("VOLTRA_METRICS_PORT")
        .and_then(|v| v.parse().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.metrics_port = p;
    }
    if let Ok(c) = env::var("VOLTRA_MAX_CONNECTIONS")
        .and_then(|v| v.parse().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.max_connections = c;
    }
    if let Ok(t) = env::var("VOLTRA_REDUCER_TIMEOUT_MS")
        .and_then(|v| v.parse().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.reducer_timeout_ms = t;
    }
    if let Ok(k) = env::var("VOLTRA_API_KEY") {
        cfg.api_key = Some(k);
    }
    if let Ok(l) = env::var("RUST_LOG") {
        cfg.log_level = l;
    }
    if let Ok(t) = env::var("VOLTRA_TUNE_SYSTEM") {
        cfg.tune_system = t == "1" || t.eq_ignore_ascii_case("true");
    }
    if let Ok(r) = env::var("VOLTRA_REUSE_PORT") {
        cfg.reuse_port = r == "1" || r.eq_ignore_ascii_case("true");
    }
    if let Ok(v) = env::var("VOLTRA_TWO_FRAME_PROTOCOL") {
        cfg.two_frame_protocol = v == "1" || v.eq_ignore_ascii_case("true");
    }
    if let Ok(i) = env::var("VOLTRA_SNAPSHOT_INTERVAL")
        .and_then(|v| v.parse().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.snapshot_interval = i;
    }
    if let Ok(d) = env::var("VOLTRA_SNAPSHOT_DIR") {
        cfg.snapshot_dir = PathBuf::from(d);
    }
    // VOLTRA_PERMISSIONS accepts a JSON object: {"delete_player":["admin"],"increment":["user","admin"]}
    ...
    if let Ok(t) = env::var("VOLTRA_SQL_TIMEOUT_MS")
        .and_then(|v| v.parse().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.sql_timeout_ms = t;
    }
    if let Ok(b) = env::var("VOLTRA_MAX_BLOB_SIZE")
        .and_then(|v| v.parse().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.max_blob_size_bytes = b;
    }
    if let Ok(m) = env::var("VOLTRA_REDUCER_MAX_MEMORY_BYTES")
        .and_then(|v| v.parse().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.reducer_max_memory_bytes = m;
    }
    if let Ok(i) = env::var("VOLTRA_REDUCER_MAX_IO_BYTES")
        .and_then(|v| v.parse().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.reducer_max_io_bytes = i;
    }
    if let Ok(c) = env::var("VOLTRA_RATE_LIMIT_CAPACITY")
        .and_then(|v| v.parse().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.rate_limit_capacity = c;
    }
    if let Ok(r) = env::var("VOLTRA_RATE_LIMIT_RATE")
        .and_then(|v| v.parse().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.rate_limit_refill_rate = r;
    }
    if let Ok(h) = env::var("VOLTRA_PRESENCE_HEARTBEAT_TIMEOUT_MS")
        .and_then(|v| v.parse().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.presence_heartbeat_timeout_ms = h;
    }
    if let Ok(o) = env::var("VOLTRA_PRESENCE_OFFLINE_TIMEOUT_MS")
        .and_then(|v| v.parse().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.presence_offline_timeout_ms = o;
    }
    if let Ok(t) = env::var("VOLTRA_TTL_SWEEP_INTERVAL_MS")
        .and_then(|v| v.parse().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.ttl_sweep_interval_ms = t;
    }
    if let Ok(p) = env::var("VOLTRA_EVICTION_POLICY") {
        cfg.eviction.policy = p;
    }
    if let Ok(n) = env::var("VOLTRA_MAX_ROWS_PER_TABLE")
        .and_then(|v| v.parse().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.eviction.max_rows_per_table = n;
    }
    if let Ok(b) = env::var("VOLTRA_MAX_BYTES_TOTAL")
        .and_then(|v| v.parse().map_err(|_| std::env::VarError::NotPresent))
    {
    ...
    if let Ok(p) = env::var("VOLTRA_TLS_CERT_PATH") {
        cfg.tls.cert_path = Some(PathBuf::from(p));
    }
    if let Ok(p) = env::var("VOLTRA_TLS_KEY_PATH") {
        cfg.tls.key_path = Some(PathBuf::from(p));
    }
    if let Ok(p) = env::var("VOLTRA_PERSISTENCE_PATH") {
        cfg.persistence_path = Some(PathBuf::from(p));
    }
    if let Ok(r) = env::var("VOLTRA_ROLE") {
        cfg.role = r;
    }
    if let Ok(u) = env::var("VOLTRA_PRIMARY_URL") {
        cfg.primary_url = Some(u);
    }
    if let Ok(ms) = env::var("VOLTRA_REPLICA_POLL_MS")
        .and_then(|v| v.parse::<u64>().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.replica_poll_ms = ms.max(50);
    }
    if let Ok(v) = env::var("VOLTRA_AUTO_FAILOVER") {
        let v = v.trim().to_ascii_lowercase();
        cfg.auto_failover = v == "1" || v == "true" || v == "yes";
    }
    if let Ok(mc) = env::var("VOLTRA_FAILOVER_MISS_COUNT")
        .and_then(|v| v.parse::<u32>().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.failover_miss_count = mc.max(1);
    }
    if let Ok(d) = env::var("VOLTRA_BACKUP_DIR") {
        cfg.backup_dir = Some(PathBuf::from(d));
    }
    if let Ok(i) = env::var("VOLTRA_BACKUP_INTERVAL_SECS")
        .and_then(|v| v.parse::<u64>().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.backup_interval_secs = i;
    }
    ...
    if let Ok(p) = env::var("VOLTRA_REDIS_PASSWORD") {
        if !p.is_empty() {
            cfg.redis_password = Some(p);
        }
    }
    if let Ok(p) = env::var("VOLTRA_PG_PORT")
        .and_then(|v| v.parse::<u16>().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.pg_port = p;
    }
    if let Ok(p) = env::var("VOLTRA_PG_PASSWORD") {
        if !p.is_empty() {
            cfg.pg_password = Some(p);
        }
    }
    if let Ok(t) = env::var("VOLTRA_SUB_TICK_MS")
        .and_then(|v| v.parse::<u64>().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.sub_tick_ms = t;
    }
    if let Ok(t) = env::var("VOLTRA_LOBBY_TICK_HZ")
        .and_then(|v| v.parse::<u32>().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.lobby_tick_hz = t;
    }
    if let Ok(d) = env::var("VOLTRA_LOBBY_VIEW_DISTANCE")
        .and_then(|v| v.parse::<f32>().map_err(|_| std::env::VarError::NotPresent))
    {
        cfg.lobby_view_distance = d;
    }
    if let Ok(r) = env::var("VOLTRA_REGION") {
        cfg.region = r;
    }
    if let Ok(r) = env::var("VOLTRA_REGIONS") {
        cfg.regions = r;
    }
    if let Ok(b) = env::var("VOLTRA_LEADERBOARD_BOARD") {
        cfg.leaderboard_board = b;
    }
    if let Ok(s) = env::var("VOLTRA_LEADERBOARD_INTERVAL_SECS")
        .and_then(|v| v.parse::<u64>().map_err(|_| std::env::VarError::NotPresent))
    {
...
fn find_config_in_cwd() -> Option<PathBuf> {
    ...
    loop {
        let candidate = dir.join("voltra.toml");
        if candidate.exists() {
