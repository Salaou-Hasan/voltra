...
fn sim_move(ctx: &mut ReducerContext, args: &[u8]) -> voltra::error::Result<Vec<u8>> {
    ...
    let mut p = match ctx.get_row("sim_players", pid)? {
        Some(v) => v,
        None => return Ok(rmp_serde::to_vec(&json!({"error": "no_player"}))?),
    };
    ...
    Ok(rmp_serde::to_vec(
        &json!({"ok": true, "x": x, "y": y, "zone": zone}),
    )?)
}
...
fn sim_attack(ctx: &mut ReducerContext, args: &[u8]) -> voltra::error::Result<Vec<u8>> {
    ...
    let mut npc = match ctx.get_row("sim_npcs", tid)? {
        Some(v) => v,
        None => return Ok(rmp_serde::to_vec(&json!({"ok": true, "skipped": true}))?),
    };
    ...
    Ok(rmp_serde::to_vec(
        &json!({"ok": true, "npc_hp": hp, "dead": hp == 0}),
    )?)
}
...
fn sim_ability(ctx: &mut ReducerContext, args: &[u8]) -> voltra::error::Result<Vec<u8>> {
    ...
    if cost > 0 && mp < cost {
        return Ok(rmp_serde::to_vec(&json!({"error": "no_mp"}))?);
    }
    ...
    Ok(rmp_serde::to_vec(
        &json!({"ok": true, "ability": ability, "mp": new_mp, "hp": p["hp"]}),
    )?)
}
...
fn sim_damage(ctx: &mut ReducerContext, args: &[u8]) -> voltra::error::Result<Vec<u8>> {
    ...
    if !p["alive"].as_bool().unwrap_or(true) {
        return Ok(rmp_serde::to_vec(&json!({"ok": true, "skipped": true}))?);
    }
...
fn sim_buy(ctx: &mut ReducerContext, args: &[u8]) -> voltra::error::Result<Vec<u8>> {
    ...
    if currency < cost {
        return Ok(rmp_serde::to_vec(&json!({"error": "insufficient"}))?);
    }
    p["currency"] = json!(currency - cost);
    ctx.set_row("sim_players".into(), pid.into(), p)?;
    let key = format!("{}:{}", pid, item);
    let mut inv = ctx
        .get_row("sim_inventory", &key)?
        .unwrap_or(json!({"pid": pid, "item": item, "qty": 0}));
    inv["qty"] = json!(inv["qty"].as_i64().unwrap_or(0) + qty);
    ctx.set_row("sim_inventory".into(), key, inv.clone())?;
    Ok(rmp_serde::to_vec(
        &json!({"ok": true, "currency": currency - cost, "qty": inv["qty"]}),
    )?)
}
...
fn sim_sell(ctx: &mut ReducerContext, args: &[u8]) -> voltra::error::Result<Vec<u8>> {
    ...
    if new_qty <= 0 {
        ctx.delete_row("sim_inventory".into(), key.clone())?;
    } else {
        let mut inv2 = inv;
        inv2["qty"] = json!(new_qty);
...
fn sim_loot(ctx: &mut ReducerContext, args: &[u8]) -> voltra::error::Result<Vec<u8>> {
    ...
    ctx.set_row("sim_inventory".into(), key, inv.clone())?;
    Ok(rmp_serde::to_vec(
        &json!({"ok": true, "item": item, "qty": inv["qty"], "rarity": rarity}),
    )?)
}
...
fn sim_transfer(ctx: &mut ReducerContext, args: &[u8]) -> voltra::error::Result<Vec<u8>> {
    ...
    let mut pf = match ctx.get_row("sim_players", from)? {
        Some(v) => v,
        None => return Ok(rmp_serde::to_vec(&json!({"error":"missing_player"}))?),
    };
    let mut pt = match ctx.get_row("sim_players", to)? {
        Some(v) => v,
        None => return Ok(rmp_serde::to_vec(&json!({"error":"missing_player"}))?),
    };
    if pf["currency"].as_i64().unwrap_or(0) < amount {
        return Ok(rmp_serde::to_vec(&json!({"error":"insufficient"}))?);
    }
...
fn sim_quest_accept(ctx: &mut ReducerContext, args: &[u8]) -> voltra::error::Result<Vec<u8>> {
    ...
    ctx.set_row(
        "sim_quests".into(),
        format!("{}:{}", pid, qid),
        json!({"pid": pid, "qid": qid, "progress": 0, "done": false}),
    )?;
    Ok(ok_bytes())
}
...
fn sim_quest_progress(ctx: &mut ReducerContext, args: &[u8]) -> voltra::error::Result<Vec<u8>> {
    ...
    let mut q = match ctx.get_row("sim_quests", &key)? {
        Some(v) => v,
        None => return Ok(rmp_serde::to_vec(&json!({"error": "no_quest"}))?),
    };
    ...
    if done {
        ...
        if let Some(mut p) = ctx.get_row("sim_players", pid)? {
            ...
            p["xp"] = json!(xp);
            p["level"] = json!(xp / 1000 + 1);
            ctx.set_row("sim_players".into(), pid.into(), p)?;
        }
    }
    ctx.set_row("sim_quests".into(), key, q)?;
    Ok(rmp_serde::to_vec(
        &json!({"ok": true, "progress": progress, "done": done}),
    )?)
}
...
fn sim_score(ctx: &mut ReducerContext, args: &[u8]) -> voltra::error::Result<Vec<u8>> {
    ...
    let cur = ctx
        .get_row("sim_leaderboard", pid)?
        .unwrap_or(json!({"pid": pid, "score": 0}));
    let best = cur["score"].as_i64().unwrap_or(0);
    if score > best {
        ctx.set_row(
            "sim_leaderboard".into(),
            pid.into(),
            json!({"pid": pid, "score": score}),
        )?;
        Ok(rmp_serde::to_vec(
            &json!({"ok": true, "new_best": true, "score": score}),
        )?)
    } else {
        Ok(rmp_serde::to_vec(
            &json!({"ok": true, "new_best": false, "score": best}),
        )?)
    }
}
...
fn sim_create_room(ctx: &mut ReducerContext, args: &[u8]) -> voltra::error::Result<Vec<u8>> {
    ...
    ctx.set_row(
        "sim_rooms".into(),
        rid.into(),
        json!({"rid": rid, "name": name, "creator": uid}),
    )?;
    Ok(ok_bytes())
}
...
fn sim_join_room(ctx: &mut ReducerContext, args: &[u8]) -> voltra::error::Result<Vec<u8>> {
    ...
    ctx.set_row(
        "sim_members".into(),
        member_key,
        json!({"rid": rid, "uid": uid, "joined": true}),
    )?;
    Ok(ok_bytes())
}
...
fn sim_send_msg(ctx: &mut ReducerContext, args: &[u8]) -> voltra::error::Result<Vec<u8>> {
    ...
    ctx.set_row(
        "sim_messages".into(),
        mid.clone(),
        json!({"mid": &mid, "rid": rid, "uid": uid, "text": text, "slot": slot}),
    )?;
...
fn sim_react(ctx: &mut ReducerContext, args: &[u8]) -> voltra::error::Result<Vec<u8>> {
    ...
    ctx.set_row(
        "sim_reactions".into(),
        key,
        json!({"mid": mid, "uid": uid, "emoji": emoji}),
    )?;
    Ok(ok_bytes())
}
...
fn sim_typing(ctx: &mut ReducerContext, args: &[u8]) -> voltra::error::Result<Vec<u8>> {
    ...
    if typing {
        ctx.set_row(
            "sim_typing".into(),
            key,
            json!({"uid": uid, "rid": rid, "ts": 0}),
        )?;
    } else {
        ctx.delete_row("sim_typing".into(), key)?;
    }
    Ok(ok_bytes())
}
...
fn sim_presence(ctx: &mut ReducerContext, args: &[u8]) -> voltra::error::Result<Vec<u8>> {
    ...
    ctx.set_row(
        "sim_presence".into(),
        uid.into(),
        json!({"uid": uid, "status": status, "ts": 0}),
    )?;
    Ok(ok_bytes())
}
...
#[command(
    name = "voltra-sim",
    about = "Voltra high-end real-world simulation benchmark"
)]
struct Args {
...
impl Metrics {
    fn new(op_names: &[&'static str]) -> Self {
        ...
        for &n in op_names {
            ops.insert(n, Arc::new(OpMetrics::new()));
        }
        Self {
            ops,
...
fn get_memory_bytes() -> u64 {
    ...
    {
        use std::mem;
        // Windows API type names — keep the platform's casing.
...
impl WsConn {
    ...
    async fn subscribe(&mut self, query: &str) -> bool {
        ...
        let Ok(frame) = rmp_serde::to_vec(&msg) else {
            return false;
        };
            ...
            match tokio::time::timeout(Duration::from_secs(5), self.inner.next()).await {
                Ok(Some(Ok(Message::Binary(b)))) => {
                    match rmp_serde::from_slice::<voltra::network::message::ServerMessage>(&b) {
                        Ok(voltra::network::message::ServerMessage::SubscriptionAck {
                            success,
                            ..
                        }) => {
                            return success;
            ...
            match tokio::time::timeout(Duration::from_secs(5), self.inner.next()).await {
                Ok(Some(Ok(Message::Binary(b)))) => {
                    ...
                    match rmp_serde::from_slice::<SM>(&b) {
                        ...
                        Ok(
                            SM::SubscriptionDiff(_)
                            | SM::SubscriptionRoute(_)
                            | SM::SubscriptionBody(_),
                        ) => {
                            SUB_FRAMES.fetch_add(1, Ordering::Relaxed);
                            continue;
                        }
...
async fn game_user(
    ...
    let mut ws = loop {
        if let Some(c) = WsConn::connect(&url, api_key.as_deref(), id as u64).await {
            break c;
        }
    ...
    for i in 0..5 {
        ...
        let x = rng.range(0, 500);
        let y = rng.range(0, 500);
        let kind = *rng.pick(&["goblin", "orc", "skeleton", "slime", "boss"]);
        let (ok, us) = ws
            .call("sim_spawn_npc", pack(&(nid.as_str(), x, y, kind)))
            .await;
    ...
    while Instant::now() < deadline {
        ...
        if !st.spawned {
            ...
            let (ok, us) = ws
                .call("sim_spawn", pack(&(pid.as_str(), x, y, *cls)))
                .await;
        ...
        match roll {
            ...
            0..=31 => {
                st.x += rng.range(-15, 15);
                st.y += rng.range(-15, 15);
                st.x = st.x.clamp(0, 999);
                st.y = st.y.clamp(0, 999);
            ...
            32..=45 => {
                ...
                let (ok, us) = ws
                    .call(
                        "sim_attack",
                        pack(&(pid.as_str(), nid.as_str(), *weapon, dmg)),
                    )
                    .await;
            ...
            46..=55 => {
                ...
                let (ok, us) = ws
                    .call("sim_ability", pack(&(pid.as_str(), *ab, target.as_str())))
                    .await;
            ...
            56..=63 => {
                ...
                let (ok, us) = ws
                    .call("sim_damage", pack(&(pid.as_str(), dmg, "npc")))
                    .await;
            ...
            64..=70 => {
                ...
                let (ok, us) = ws
                    .call("sim_buy", pack(&(pid.as_str(), *item, qty, 10)))
                    .await;
            ...
            71..=76 => {
                ...
                let (ok, us) = ws
                    .call("sim_sell", pack(&(pid.as_str(), *item, 1, 8)))
                    .await;
            ...
            77..=81 => {
                ...
                let (ok, us) = ws
                    .call("sim_loot", pack(&(pid.as_str(), box_id.as_str(), *rarity)))
                    .await;
            ...
            82..=86 => {
                if !st.quest_active {
                    ...
                    let (ok, us) = ws
                        .call(
                            "sim_quest_accept",
                            pack(&(pid.as_str(), st.quest_id.as_str())),
                        )
                        .await;
                    metrics.record("sim_quest_accept", ok, us);
                    metrics.record_lobby(lobby, us);
                    if ok {
                        st.quest_active = true;
                    }
                } else {
                    let (ok, us) = ws
                        .call(
                            "sim_quest_progress",
                            pack(&(pid.as_str(), st.quest_id.as_str(), 1)),
                        )
                        .await;
            ...
            87..=90 => {
                if !st.queued {
                    ...
                    let (ok, us) = ws.call("sim_queue", pack(&(pid.as_str(), *mode))).await;
            ...
            91..=93 => {
                ...
                let (ok, us) = ws.call("sim_score", pack(&(pid.as_str(), score))).await;
            ...
            94..=96 => {
                if !st.peer_ids.is_empty() && st.currency > 20 {
                    ...
                    let (ok, us) = ws
                        .call("sim_transfer", pack(&(pid.as_str(), target.as_str(), amt)))
                        .await;
            ...
            97..=99 => {
                ...
                let (ok, us) = ws
                    ...
                    .await;
                metrics.record("sim_spawn_npc", ok, us);
                if ok && !st.npc_pool.contains(&nid) {
                    st.npc_pool.push(nid);
                }
            }
            _ => {}
        ...
        if think > 0 {
            ...
            let jitter = rng
                .range(think as i32 / 2, think as i32 + think as i32 / 2)
                .max(1) as u64;
...
fn geo_delay_ms(bot_id: usize) -> u64 {
    if !GEO_ENABLED.load(Ordering::Relaxed) {
        return 0;
    }
    match bot_id % 10 {
        0..=2 => 5,  // 30% — same region (~10ms RTT)
        3..=7 => 25, // 50% — cross-region (~50ms RTT)
...
async fn chat_user(
    ...
    if id.is_multiple_of(room_count.max(1)) {
        let (ok, us) = ws
            .call(
                "sim_create_room",
                pack(&(
                    room_id.as_str(),
                    format!("Room {}", id % room_count.max(1)).as_str(),
                    uid.as_str(),
                )),
            )
            .await;
        ...
        let (ok, us) = ws
            .call("sim_join_room", pack(&(room_id.as_str(), uid.as_str())))
            .await;
        metrics.record("sim_join_room", ok, us);
    }
    let (ok, us) = ws
        .call("sim_presence", pack(&(uid.as_str(), "online")))
        .await;
        ...
        match roll {
            ...
            0..=32 => {
                ...
                let (ok, us) = ws
                    .call(
                        "sim_send_msg",
                        pack(&(room_id.as_str(), mid.as_str(), uid.as_str(), text.as_str())),
                    )
                    .await;
                metrics.record("sim_send_msg", ok, us);
                if ok && recent_msgs.len() < 20 {
                    recent_msgs.push(mid);
                }
            }
            ...
            33..=60 => {
                ...
                let (ok, us) = ws
                    .call(
                        "sim_typing",
                        pack(&(uid.as_str(), room_id.as_str(), typing)),
                    )
                    .await;
            ...
            61..=74 => {
                if !recent_msgs.is_empty() {
                    ...
                    let (ok, us) = ws
                        .call("sim_react", pack(&(mid.as_str(), uid.as_str(), *emoji)))
                        .await;
            ...
            75..=86 => {
                ...
                let (ok, us) = ws
                    .call("sim_presence", pack(&(uid.as_str(), *status)))
                    .await;
                metrics.record("sim_presence", ok, us);
            }
            // 8% — thread reply
            87..=94 => {
                if !recent_msgs.is_empty() {
                    ...
                    let (ok, us) = ws
                        .call(
                            "sim_thread_reply",
                            pack(&(tid.as_str(), rid.as_str(), uid.as_str(), text.as_str())),
                        )
                        .await;
            ...
            95..=99 => {
                ...
                let (ok, us) = ws
                    .call("sim_join_room", pack(&(new_room.as_str(), uid.as_str())))
                    .await;
                metrics.record("sim_join_room", ok, us);
            }
            _ => {}
...
async fn start_stress_server(
    ...
    let dir =
        std::env::temp_dir().join(format!("voltra_stress_{}_{}", ws_port, std::process::id()));
    ...
    if extra_workers > 0 {
        ...
        std::env::set_var(
            "VOLTRA_WORKERS",
            (num_cpus::get() + extra_workers).to_string(),
        );
    } else {
    ...
    match voltra::run_server_with_handle(config).await {
        Ok((handle, server_fut)) => {
            tokio::spawn(async move {
                if let Err(e) = server_fut.await {
                    eprintln!("[stress-server] error: {e}");
                }
            });
            handle
        }
        Err(e) => {
            eprintln!("[stress-server] startup failed: {e}");
            std::process::exit(1);
        }
    }
}
...
async fn run_stress_client(
    ...
    let Some(conn) = WsConn::connect(&url, api_key.as_deref(), user_id).await else {
        return;
    };
    ...
    loop {
        let timeout_ms = if done.load(Ordering::Relaxed) {
            300
        } else {
            5000
        };
        match tokio::time::timeout(Duration::from_millis(timeout_ms), stream.next()).await {
            Ok(Some(Ok(Message::Binary(_) | Message::Text(_)))) => {
                ...
                if !done.load(Ordering::Relaxed) && !send_next!() {
                    return;
                }
            }
        ...
        if done.load(Ordering::Relaxed) && in_flight.is_empty() {
            return;
        }
    }
}
...
async fn run_stress(
    ...
    loop {
        ...
        if now >= measure_start && !measuring.load(Ordering::SeqCst) {
            ...
            if let Ok(mut h) = hist.lock() {
                *h = Histogram::new_with_bounds(1, 60_000_000, 3).unwrap();
            }
            last_calls = 0;
            last_t = Instant::now();
        ...
        if measuring.load(Ordering::Relaxed) {
            ...
            println!(
                "[{:>4}s] tps={:>9.0}  calls={:>9}  err={:>5}  mem={:.1}MB  rows={:>6}  conn={}",
                sim_start.elapsed().as_secs(),
                window_tps,
                calls,
    ...
    let err_pct = if total + errors > 0 {
        100.0 * errors as f64 / (total + errors) as f64
    } else {
        0.0
    };
    ...
    let mem_growth = if mem_first > 0.0 {
        100.0 * (mem_last - mem_first) / mem_first
    } else {
        0.0
    };
    ...
    println!("╠══════════════════════════════════════════════════════════════════════╣");
    println!(
        "║  Memory: {mem_first:.1}MB → {mem_last:.1}MB ({mem_growth:+.1}%)   Rows: {}",
        samples.last().map(|s| s.rows).unwrap_or(0)
    );
    println!("╠══════════════════════════════════════════════════════════════════════╣");
    let target_250k = if avg_tps >= 250_000.0 {
        "✅ ≥ 250K TPS"
    } else {
        "❌ < 250K TPS"
    };
    let target_400k = if avg_tps >= 400_000.0 {
        "✅ ≥ 400K TPS"
    } else {
        "❌ < 400K TPS"
    };
...
async fn run_game_sim(
    ...
    for i in 0..n_players {
        ...
        handles.push(tokio::spawn(game_user(
            i + cfg.id_offset,
            url,
            ak,
            met,
...
async fn run_chat_sim(
    ...
    for i in 0..n_users {
        ...
        handles.push(tokio::spawn(chat_user(
            i + cfg.id_offset,
            url,
            ak,
            met,
...
async fn sample_loop(
    ...
    loop {
        ...
        println!(
            "[{:>5}s] tps={:>8.0}  ok={:>9}  err={:>5}  mem={:>5.1}MB  \
                  wal={:>5.1}MB  rows={:>6}  conn={}",
            elapsed, window_tps, total, errors, h.memory_mb, h.wal_mb, h.rows, h.connections
        );
        samples.push(h);
...
fn print_lobby_summary(metrics: &Metrics) {
    if metrics.lobby_hists.is_empty() {
        return;
    }
    // (lobby, p50, p99) per instance
    let mut per: Vec<(usize, f64, f64)> = metrics
        .lobby_hists
        .iter()
        .map(|e| {
            let h = e.value().lock();
    ...
    println!(
        "║    best lobby  :  p50 {:>8.2}ms   p99 {:>8.2}ms   (l{})",
        best.1, best.2, best.0
    );
    println!(
        "║    worst lobby :  p50 {:>8.2}ms   p99 {:>8.2}ms   (l{})",
...
fn print_report(label: &str, metrics: &Metrics, samples: &[Health], elapsed_secs: f64) {
    ...
    let mut ops: Vec<(&str, &OpMetrics)> =
        metrics.ops.iter().map(|(k, v)| (*k, v.as_ref())).collect();
    ops.sort_by(|a, b| {
        b.1.calls
            .load(Ordering::Relaxed)
            .cmp(&a.1.calls.load(Ordering::Relaxed))
    });
    ...
    for (_, op) in &ops {
        ...
        if let Ok(h) = op.hist.lock() {
            let _ = all_hist.add(&*h);
        }
    }
    println!(
        "║  {:<22}  {:>8}  {:>8.1}  {:>8.2}  {:>8.2}  {:>7}",
    ...
    if let (Some(first), Some(last)) = (samples.first(), samples.last()) {
        ...
        println!("╠══════════════════════════════════════════════════════════════════════╣");
        println!(
            "║  Memory: {:.1}MB → {:.1}MB ({:+.1}%)   WAL: {:.1}MB   Rows: {}",
    ...
    if sub_frames > 0 {
        println!("╠══════════════════════════════════════════════════════════════════════╣");
        println!(
            "║  Subscription fan-out received: {} frames ({:.0}/s)",
            sub_frames,
            sub_frames as f64 / elapsed_secs.max(0.001)
        );
    }
...
async fn run_scale(
    ...
    println!(
        "\n┌─ Scale test: profile={profile} levels={:?} duration={dur}s/level ─┐",
        levels
    );
    ...
    for &n in &levels {
        ...
        let mut all_hist = Histogram::<u64>::new_with_bounds(1, 60_000_000, 3).unwrap();
        for op in metrics.ops.values() {
            if let Ok(h) = op.hist.lock() {
                let _ = all_hist.add(&*h);
            }
        }
        let (p50, p99) = if !all_hist.is_empty() {
            (
                all_hist.value_at_quantile(0.50) as f64 / 1000.0,
                all_hist.value_at_quantile(0.99) as f64 / 1000.0,
            )
...
async fn main() {
    ...
    let stats = if args.external {
        ...
        println!(
            "│  Connecting to {}   stats from {}",
            args.url, args.metrics_url
        );
        println!("└────────────────────────────────────────────────────────────────────┘\n");
    ...
    match &args.scenario {
        ScenarioCmd::Game {
            ...
            let lobbies = (*players).div_ceil(ls);
            println!("▶  GAME simulation — {players} players in {lobbies} lobbies of {ls} for {duration}s (ramp {ramp}s)");
            println!(
                "   Workload: positions, combat, abilities, economy, quests, matchmaking, world"
            );
            if args.geo {
        ...
        ScenarioCmd::Mixed {
            ...
            println!(
                "▶  MIXED simulation — {players} game players + {users} chat users for {duration}s"
            );
            let metrics = Arc::new(Metrics::new(&all_ops));
            let t0 = Instant::now();
...
fn write_csv(path: &str, samples: &[Health]) {
    ...
    let rows: Vec<String> = samples
        ...
        .map(|h| {
            format!(
                "{},{:.2},{:.2},{},{}",
                h.elapsed_secs, h.memory_mb, h.wal_mb, h.rows, h.connections
            )
        })
        .collect();
    let content = format!("{}\n{}", header, rows.join("\n"));
