...
use crate::runtime::aoi_broadcast::AoiBroadcaster;
...
pub struct LobbyRouter {
    ...
    /// Per-lobby AOI broadcasters (created when the lobby worker spawns).
    aoi_broadcasters: DashMap<String, Arc<AoiBroadcaster>>,
}
...
pub struct WorkerDeps {
    ...
    pub lobby_tick_config: Option<crate::runtime::tick::TickConfig>,
...
impl LobbyRouter {
    ...
    pub async fn dispatch(&self, call: PendingCall) -> std::result::Result<(), String> {
        ...
        let broadcaster = Arc::new(AoiBroadcaster::new());
        self.aoi_broadcasters.insert(lobby_id.clone(), broadcaster);
    ...
    /// Get the AOI broadcaster for a specific lobby.
    pub fn get_aoi_broadcaster(&self, lobby_id: &str) -> Option<Arc<AoiBroadcaster>> {
        self.aoi_broadcasters.get(lobby_id).map(|e| e.value().clone())
    }
...
fn lobby_worker_loop(
    ...
    use crate::runtime::lobby_runtime::LobbyRuntime;
    ...
    let mut lobby_rt: Option<LobbyRuntime> = deps.lobby_tick_config.clone().map(|config| {
        log::info!(
            "[lobby-{}] Runtime started — tick {}Hz, view_distance {}",
            lobby_id, config.tick_rate_hz, deps.lobby_view_distance
        );
        LobbyRuntime::new(lobby_id.to_string(), config, deps.lobby_view_distance)
    });
    ...
    loop {
        ...
        // Run game simulation tick if the runtime is active and a tick is due.
        if let Some(ref mut lrt) = lobby_rt {
            if let Some(result) = lrt.tick_if_due() {
                if result.aoi_updates > 0 {
                    log::trace!(
                        "[lobby-{}] tick {} — {} entities, {} aoi events, {:.2}µs",
                        lobby_id, result.tick_number, result.entities_processed,
                        result.aoi_updates, result.elapsed_ns as f64 / 1000.0
                    );
                }
            }
        }
