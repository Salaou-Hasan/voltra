# NeonDB Operations Runbook

Day-2 operations for a running NeonDB instance: backups, health checks, disaster
...
NeonDB persistence is a two-layer system:

1. **WAL** (`neondb.wal` by default) — every committed reducer call appends one
...
2. **Snapshots** — a full dump of `TableStore` to `snapshot_dir/snapshot_<seq>.bin`
   every `snapshot_interval` WAL entries (default 100,000 in `neondb.toml`).
...
| --------------- | ---------------------------- | ------------------------- |
| WAL file        | `$TEMP/neondb.wal`           | `wal_path` / `NEONDB_WAL_PATH` |
| Snapshot dir    | `$TEMP/neondb_snapshots/`    | `snapshot_dir` / `NEONDB_SNAPSHOT_DIR` |
| Blob store dir  | `$TEMP/neondb_blobs/`        | `NEONDB_BLOB_PATH`        |
...
1. Reduce `snapshot_interval` temporarily via `NEONDB_SNAPSHOT_INTERVAL=1`.
2. Issue any reducer call (e.g. `neondb call noop`).
...
NeonDB exposes a metrics HTTP server on `metrics_port` (default `port + 1` =
`3001`).
...
BACKUP_DIR=/backup/neondb/$DATE
mkdir -p "$BACKUP_DIR"

# 1. Snapshot directory (multiple files OK — restore uses the highest seq).
cp -a /var/lib/neondb/snapshots "$BACKUP_DIR/"
...
cp /var/lib/neondb/neondb.wal "$BACKUP_DIR/"

# 3. Optional: blob store (only if you write large blobs via BlobStore).
cp /var/lib/neondb/blobs.bin "$BACKUP_DIR/" 2>/dev/null || true

# 4. Config — restoring without it is a coin-flip.
cp /etc/neondb/neondb.toml "$BACKUP_DIR/"

# 5. Compress + ship offsite.
tar -czf "$BACKUP_DIR.tar.gz" -C /backup/neondb "$DATE"
rm -rf "$BACKUP_DIR"
```
...
# 2. Untar the backup.
tar -xzf /backup/neondb/20260607_030000.tar.gz -C /tmp/restore

# 3. Place files where the config expects them.
mkdir -p /var/lib/neondb
cp -a /tmp/restore/*/snapshots /var/lib/neondb/
cp    /tmp/restore/*/neondb.wal /var/lib/neondb/
cp    /tmp/restore/*/blobs.bin  /var/lib/neondb/ 2>/dev/null || true
cp    /tmp/restore/*/neondb.toml /etc/neondb/

# 4. Start. The server auto-loads the latest snapshot and replays the WAL.
systemctl start neondb
journalctl -u neondb -f   # watch replay finish
...
| ---------------------------------- | ------------------------------ | ------------------------ | ----- |
| `NEONDB_HOST`                      | `host`                         | `127.0.0.1`              | Bind address. `0.0.0.0` for external. |
| `NEONDB_PORT`                      | `port`                         | `3000`                   | WebSocket port. |
| `NEONDB_METRICS_PORT`              | `metrics_port`                 | `port + 1`               | HTTP metrics port. |
| `NEONDB_WAL_PATH`                  | `wal_path`                     | `$TEMP/neondb.wal`       | Set explicitly in prod. |
| `NEONDB_FSYNC_INTERVAL_MS`         | `fsync_interval_ms`            | `100`                    | 0 = fsync per write. |
| `NEONDB_WAL_BATCH_SIZE`            | `wal_batch_size`               | `100000`                 | Entries per flush. |
| `NEONDB_WAL_BATCH_INTERVAL_MS`     | `wal_batch_interval_ms`        | `100`                    | Flush ceiling. |
| `NEONDB_UNSAFE_NO_FSYNC`           | `unsafe_no_fsync`              | `false`                  | `1` = never fsync. Benchmarks only. |
| `NEONDB_SNAPSHOT_INTERVAL`         | `snapshot_interval`            | `100000`                 | Entries between snapshots. |
| `NEONDB_SNAPSHOT_DIR`              | `snapshot_dir`                 | `$TEMP/neondb_snapshots` | Persistent dir in prod. |
| `NEONDB_SHARD_ID`                  | `shard_id`                     | `0`                      | This node's shard index. |
| `NEONDB_SHARD_COUNT`               | `shard_count`                  | `1`                      | Total shards in cluster. |
| `NEONDB_MAX_CONNECTIONS`           | `max_connections`              | `500`                    | Concurrent WebSocket clients. |
| `NEONDB_REDUCER_TIMEOUT_MS`        | `reducer_timeout_ms`           | `5000`                   | Per-call timeout. |
| `NEONDB_API_KEY`                   | `api_key`                      | (none)                   | Bearer token. **REQUIRED in prod.** |
| `NEONDB_TUNE_SYSTEM`               | `tune_system`                  | `false`                  | Linux: raise file descriptor limit. |
| `NEONDB_REUSE_PORT`                | `reuse_port`                   | `true`                   | SO_REUSEPORT. |
| `NEONDB_TWO_FRAME_PROTOCOL`        | `two_frame_protocol`           | `false`                  | Optional shared-body delta encoding. |
| `NEONDB_SQL_TIMEOUT_MS`            | `sql_timeout_ms`               | `5000`                   | Admin SQL query timeout. |
| `NEONDB_MAX_BLOB_SIZE`             | `max_blob_size_bytes`          | `16777216` (16 MiB)      | Reject larger blobs. |
| `NEONDB_BLOB_PATH`                 | (none)                         | `$TEMP/neondb_blobs`     | Blob store directory. |
| `NEONDB_PERMISSIONS`               | `[permissions]`                | (open)                   | JSON `{"reducer":["role"]}`. |
| `NEONDB_PERMISSIONS_DEFAULT_POLICY`| `permissions_default_policy`   | `open`                   | `open` or `closed`. |
...
| Survive Hugmany clients         | `max_connections` ↑, `NEONDB_TUNE_SYSTEM=1` | Plus OS `ulimit -n 65535`. |
