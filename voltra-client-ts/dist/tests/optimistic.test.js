    ...
    return new Promise((resolve) => {
        ...
        const handle = {
            ...
            pushDiff: (subId, table, key, op, data) => {
                if (!openSocket)
                    return;
                openSocket.send(encode({ SubscriptionDiff: [subId, table, key, op, data] }));
            },
            close: () => wss.close(),
        };
...
test("optimistic: error preserves mid-flight subscription diffs on untouched rows", async () => {
    ...
    await waitUntil(() => client.getRow("enemies", "goblin1")?.hp === 50);
    ...
    assert.equal(client.getRow("players", "alice"), undefined, "alice should be rolled back (touched row)");
    ...
    assert.deepEqual(client.getRow("enemies", "goblin1"), { hp: 50 }, "goblin1 must NOT be rolled back to pre-call value — it was changed by a subscription diff mid-flight");
...
test("optimistic: callback returns identical cache → empty touched set, no-op rollback", async () => {
    ...
    const callPromise = client.call("noop_reducer", [], {
        optimistic: (cache) => {
...
//# sourceMappingURL=optimistic.test.js.map