...
test("computeBackoffDelay: jitter stays within ±25% of capped base", () => {
    ...
    for (let attempt = 0; attempt < 6; attempt++) {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            assert.ok(delay >= low && delay <= high, `attempt=${attempt}: delay ${delay} not in [${low}, ${high}]`);
        }
    }
});
test("computeBackoffDelay: maxDelayMs is respected even with jitter", () => {
    ...
    for (let i = 0; i < 30; i++) {
        ...
        assert.ok(d >= Math.round(maxDelay * 0.75) && d <= Math.round(maxDelay * 1.25), `d=${d} not in jitter-allowed range around maxDelay=${maxDelay}`);
    }
});
