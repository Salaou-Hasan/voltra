...
function resolveReconnect(opts: VoltraClientOptions): {
  enabled: boolean;
  maxAttempts: number;
  baseDelayMs: number;
  maxDelayMs: number;
...
export class VoltraClient {
  private readonly opts: Required<VoltraClientOptions>;
  ...
  private handleFrame(data: ArrayBuffer | Uint8Array): void {
    ...
    switch (msg.type) {
      ...
      case "SubscriptionAck":
        if (!msg.data.success) {
          console.warn(
            `[Voltra] Subscription "${msg.data.subscriptionId}" failed: ${msg.data.message}`,
          );
        }
        break;
